 ...
 * cg_descent (x, n, NULL, NULL, grad_tol, user_value, user_gradient,
 * user_value_gradient, NULL) ; where n is x.size(), grad_tol is a float must
 * have signature
 *
...
inline int pycgd_test_callback(double f, double *x, double *g, INT n,
                               void *user_data);
...
public:
  CGDescent(std::shared_ptr<pele::BasePotential> potential,
            const pele::Array<double> x0, double tol = 1e-4,
            size_t PrintLevel = 0)
      : m_pot(potential), m_parm(), m_stats(), m_x0(x0.copy()), m_x(x0.copy()),
        m_tol(tol), m_nfev(0), m_success(false) {
  ...
  inline void run() {
    ...
    // using ::cg_descent call in the global namespace (the one in
    // CG_DESCENT 6.8), this resolves the ambiguity
    INT cgout = ::cg_descent(m_x.data(), m_x.size(), &m_stats, &m_parm, m_tol,
                             pycgd_value, pycgd_gradient, pycgd_value_gradient,
                             NULL, pycgd_test_callback, (void *)this);
  ...
  inline bool test_success(INT cgout) {
    if (cgout == 0) {
      ...
      case 8:
        ...
        break;
      case 9:
        std::cout << "debugger is on and the function value increases"
                  << std::endl;
        break;
      case 10:
        ...
        break;
      case 11:
        std::cout << "function nan or +-INF and could not be repaired"
                  << std::endl;
        break;
      case 12:
